...
var nora_engine = function(){
    ...
    var narrative_position_iterator = make_narrative_position_iterator(),
        current_narrative_position = narrative_position_iterator.value();
    ...
    var compute_next_asset = function(){
        ...
        // invalid state
        if(current_narrative_position == null){
            return null;
        }
        ...
        var compute_asset_pool = function(){
             ...
             var pool = nora_db.assets_with_zone_id_and_narrative_position(zone_id, current_narrative_position);
        ...
        while(asset_pool.length == 0){
            // no available media of type current_narrative_position in current_zone
            // increment narrative position
            current_narrative_position = narrative_position_iterator.increment_narrative_position().value();

            if(current_narrative_position === null){
                console.log("narrative positions exhausted");
                return null;
            }   
            console.log("narr position", current_narrative_position);
            asset_pool = compute_asset_pool();
        }

        var asset = _.chain(asset_pool).shuffle().head(1).value();

        narrative_position = narrative_position_iterator.next().value();
